 ...
 * __________________
 * 
 *  Copyright [2009] - [2018] Digital Alchemy, Incorporated 
 *  All Rights Reserved.
 * 
  ...
  public class ClipperOffset
  {
    ...
    void OffsetPoint(int j, ref int k, JoinType jointype)
    {
      ...
      if (Math.Abs(m_sinA * m_delta) < 1.0) 
      {
        ...
        {
          IntPoint ip = new IntPoint(Round(m_srcPoly[j].X + m_normals[k].X * m_delta),
            Round(m_srcPoly[j].Y + m_normals[k].Y * m_delta));
          ip.userData = m_srcPoly[j].userData;
          m_destPoly.Add(ip);
          return; 
        }
      ...
      if (m_sinA * m_delta < 0)
      {
        IntPoint ip1 = new IntPoint(Round(m_srcPoly[j].X + m_normals[k].X * m_delta),
          Round(m_srcPoly[j].Y + m_normals[k].Y * m_delta));
        ip1.userData = m_srcPoly[j].userData;
        m_destPoly.Add(ip1);
        m_destPoly.Add(m_srcPoly[j]);
          IntPoint ip2 = new IntPoint(Round(m_srcPoly[j].X + m_normals[j].X * m_delta),
          Round(m_srcPoly[j].Y + m_normals[j].Y * m_delta));
          ip2.userData = m_srcPoly[j].userData;
          m_destPoly.Add(ip2);
      }
      else
        switch (jointype)
